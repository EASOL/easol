...
class External_Auth extends CI_Model {
    ...
    public static function validate_google_token($useremail, $gidtoken, $environment)
    {
	    ...
	    if($output === FALSE) {
		    ...
		    $goodg = 'yes';
		    if(isset($tokarray->iss) && $tokarray->iss=="accounts.google.com") { 
      } else { $goodg = 'no'; 
      }
		    if(isset($tokarray->aud) && $tokarray->aud=="1046550702050-or91v65jm72mmdv8tjesehm3qbq3d4ol.apps.googleusercontent.com") {
      } else { $goodg = 'no'; 
      }
		    
		    if( isset($tokarray->email) && $tokarray->email==$useremail && $tokarray->email_verified=="true") { 
      } else { $goodg = 'no'; 
      }
		    
		    if($goodg == 'yes') { return 'valid';

      } else { return "Invalid Value"; /* var_dump($tokarray); */ 
      }
	    }   
